...
class Evaluator:
    ...
    @classmethod
    def compute_metrics(cls, area_inter, area_union, area_pred, area_gt):
        """
        基于累计的区域面积计算 IoU 和 Dice
        Dice 依据: 2 * Intersection / (Area_Pred + Area_GT)
        """
        # 忽略背景类 (index 0)，只关心前景类 (index 1)
        # 这里假设 area_* 的 shape 是 [2, N] 或者 [2]，其中索引 1 是前景
        
        # Intersection over Union
        # 添加 epsilon 防止除零
        iou = area_inter[1] / (area_union[1] + 1e-10)
        
        # Dice Coefficient
        # Dice = 2 * TP / (TP + FP + TP + FN) = 2 * Inter / (Pred_Area + GT_Area)
        dice = 2 * area_inter[1] / (area_pred[1] + area_gt[1] + 1e-10)
